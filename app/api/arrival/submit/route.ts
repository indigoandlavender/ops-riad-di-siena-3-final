...
export async function POST(request: NextRequest) {
  try {
    ...
    const { bookingId, arrivalTime } = body;

    if (!bookingId || !arrivalTime) {
      return NextResponse.json(
        { error: "Missing booking ID or arrival time" },
        { status: 400 }
      );
    }
    ...
    // Find header row and booking_id column
    const headers = rows[0];
    const bookingIdCol = headers.findIndex(
      (h: string) => h.toLowerCase().replace(/[_\s]/g, "") === "bookingid"
    );
    const arrivalTimeConfirmedCol = headers.findIndex(
      (h: string) => h.toLowerCase().replace(/[_\s]/g, "") === "arrivaltimeconfirmed"
    );
    ...
    if (arrivalTimeConfirmedCol === -1) {
      return NextResponse.json(
        { error: "Arrival time column not configured. Please add 'arrival_time_confirmed' column to Master_Guests." },
        { status: 500 }
      );
    }

    // Update the arrival_time_confirmed cell
    const colLetter = String.fromCharCode(65 + arrivalTimeConfirmedCol);
    ...
    return NextResponse.json(
      { error: "Failed to save arrival time" },
      { status: 500 }
    );
  }
}
